...
int main( int argc, char *argv[] )
{
  int i=0 ,	// loop index
		...
		maxItems=0 ,  // maximum allowed number of items (user-supplied)
  ...
  if( argc < 4 || maxItems < 1 || maxItems > n )
	 maxItems = n ;
  printf( "Max number of items in answer is %u \n", maxItems );
...
void bound( node *x, const nodeArray pw, const int n, const int W, const int maxItems )
{
  ...
  printf( "\n bound(1): n == %d ; maxItems == %d \n", n, maxItems );
  displayNode( x );
#endif

  // calculate the new bound if weight and num items are under their limits
  if( totweight < W  &&  items <= maxItems )
  {
	 result = (float)x->profit ;

	 levels = x->level + 1 ;
	 #if KNAPSACK_BOUND_DEBUG > 0
	 printf( " bound(2): starting levels == %d \n", levels );
	 #endif

	 // grab as many items as possible
	 while( items < maxItems  &&  levels < n  &&  (totweight + pw[levels].weight <= W) )
	 {
		totweight += pw[levels].weight ;
		result += (float)pw[levels].profit ;
		#if KNAPSACK_BOUND_DEBUG > 0
		printf( " bound(3): levels == %d ; result == %7.3f \n", levels, result );
		#endif
		levels++ ;
		items++ ;
	 }

	 if( items <= maxItems  &&  levels < n )
		// grab fraction of jth item
		result += ( ((float)(W - totweight)) * pw[levels].pw );
	 ...
	 if( v.bound > maxprofit ) // check if node is still promising
	 {
		...
		if( u.weight <= W  &&  u.profit > maxprofit  &&  u.numitems <= maxItems )
		{
		  ...
		  *tw = u.weight ;
		  ...
		  printf( "\t current number of items is %d \n", u.numitems );
		  printf( "\n**********************************************************\n");
