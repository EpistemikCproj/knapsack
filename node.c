...
{
  if( x->name )
    free( x->name ); // free any existing char* memory
...
void displayNode( nodePtr x )
{
  ...
  printf( " items %2d ; level %2d ; bound == %8.3f ;", x->numitems, x->level, x->bound );
...
void deleteQueue( PqPtr p )
{
  if( p->nodes == NULL && p->size == 0 )
    ...
    printf( "deleteQueue(): ERROR: p->nodes == %p and p->size == %d !!", p->nodes, p->size );
